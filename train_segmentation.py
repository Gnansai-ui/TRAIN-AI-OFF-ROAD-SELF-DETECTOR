...
Trains a DeepLabV3+ segmentation head on top of DINOv2 backbone
"""
...
# ============================================================================
# Model: DeepLabV3+ Head on top of DINOv2 patch tokens
# ============================================================================

def gn(num_channels, num_groups=32):
    """GroupNorm helper — works with any batch size, including batch=1."""
    # Clamp groups so num_channels is always divisible
...
class DeepLabV3PlusHead(nn.Module):
    """
    DeepLabV3+ decoder head designed for DINOv2 patch token features.

    DINOv2 produces flat patch tokens of shape (B, N, C).
    This head:
      1. Reshapes tokens back to a 2D feature map
      2. Applies ASPP for multi-scale context
      3. Decodes with a lightweight low-level feature branch
      4. Outputs per-class logits at patch resolution
         (caller upsamples to full image size)
    """
    def __init__(self, in_channels, num_classes, tokenH, tokenW,
        ...
        # Project DINOv2 features to spatial map
        ...
        # Low-level feature branch (projects same input at 1/4 scale analogue)
    ...
    def forward(self, x):
        ...
        returns: (B, num_classes, tokenH, tokenW) logits
        """
        B, N, C = x.shape
        # Reshape to spatial feature map
        feat = x.reshape(B, self.H, self.W, C).permute(0, 3, 1, 2)  # (B, C, H, W)

        # Low-level features (skip connection)
        low = self.low_level_proj(feat)                               # (B, 48, H, W)

        # High-level features via ASPP
        high = self.input_proj(feat)                                  # (B, 256, H, W)
        high = self.aspp(high)                                        # (B, 256, H, W)

        # Concatenate and decode
        combined = torch.cat([high, low], dim=1)                      # (B, 304, H, W)
        return self.decoder(combined)                                 # (B, num_classes, H, W)
...
def main():
    ...
    # Dataset paths
    data_dir = 'C:/falcon data set/Offroad_Segmentation_Training_Dataset/train'
    val_dir  = 'C:/falcon data set/Offroad_Segmentation_Training_Dataset/val'
    ...
    # -----------------------------------------------------------------------
    # Load DINOv2 backbone (unchanged)
    # -----------------------------------------------------------------------
    ...
    print(f"\nArchitecture: DINOv2-{BACKBONE_SIZE} + DeepLabV3+ head")
    print(f"Segmentation classes: {n_classes}")

    # -----------------------------------------------------------------------
    # Loss: CrossEntropy  ← CHANGED: label smoothing for better generalisation
    # -----------------------------------------------------------------------
    ...
    # -----------------------------------------------------------------------
    # LR Scheduler: cosine annealing  ← NEW
    # -----------------------------------------------------------------------
    scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=n_epochs, eta_min=1e-6)

    # -----------------------------------------------------------------------
    # Best-model checkpoint  ← NEW
    # -----------------------------------------------------------------------
    best_val_iou = 0.0
    ...
    for epoch in epoch_pbar:
        ...
        if val_iou > best_val_iou:
            ...
            tqdm.write(f"  ✓ New best val IoU: {best_val_iou:.4f} — checkpoint saved")
